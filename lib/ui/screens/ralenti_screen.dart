...
final _metrics = [
  MetricDef(
    ...
    unit: 'rpm',
    maxValue: 2000,
    getValue: (d) => d?.idleSetpoint?.toDouble() ?? 0,
  ),
  MetricDef(
    label: 'Vanne',
    unit: '%',
    maxValue: 100,
    getValue: (d) => d?.idleValvePosition?.toDouble() ?? 0,
  ),
  MetricDef(
    label: 'Base',
    unit: '%',
    maxValue: 100,
    getValue: (d) => d?.idleBasePosition?.toDouble() ?? 0,
  ),
  MetricDef(
    label: 'Erreur',
    unit: 'rpm',
    maxValue: 300,
    getValue: (d) => d?.idleError?.toDouble() ?? 0,
  ),
  MetricDef(
    label: 'Ajusteur',
    unit: 'rpm',
    maxValue: 300,
    getValue: (d) => d?.idleAdjusterRpm?.toDouble() ?? 0,
  ),
];
